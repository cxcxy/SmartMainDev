...
//    var dataSourceArray: [ConfigNetModel]?
    var dataSourceArray:[ConfigNetModel]?{
        didSet{
            if (dataSourceArray?.count ?? 0) > 1 {// 如果大于1个，则显示pageController
                configureView()
    ...
    func singProductView() -> WOW_SingProductView {
        let v = WOW_SingProductView.loadFromNib()
...
//        self.rightView?.view_CountDown?.addSubview(cvRight)
    ...
    func resetViewSource() {
        ...
        if self.currentIndex == 0 {
            ...
            let rightImageIndex = (self.dataSourceArray?.count ?? 0) > 1 ? 1 : 0 //保护
            ...
            configureModel(model: dataSourceArray?[self.currentIndex-1],v: self.leftView)
            if self.currentIndex < dataSourceArray?.count ?? 0{
